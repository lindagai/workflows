...
Now we can define the region of interest using a `GRanges` object:
...
Looking at the above output, we can see that we have missing calls (`.`), as well as half-calls (`0/.` and `1/.`). Half-calls should be set to missing, and a delimiter should be added, since, if you plan on imputing missing values and phasing, phasing software like BEAGLE require a delimiter between the alleles to read in the VCF.
...
Note that `trio.check` can only read in entries of the form: `"0/0"`, `"0/1"`, or `"1/1"`; missing entries (`NA` or `./.`) are not accepted. `vcf2geno` automatically gets rid of SNPs with missing entries for you, but you can check the entries of the genotypes in the `trio.geno` matrix using `table()`:
...
```{r,eval = FALSE}
trio.geno <- removeSNPs(trio.geno, perc.na = 0.1)
```
