...
BiocManager::install("VariantAnnotation", "trio", "TVTB", "GViz")
...
## External software for preparing the dataset
...
This workflow uses vcftools to filter a VCF to rare variants only and to remove monomorphic SNPs.

JHPCE already has vcftools available on the cluster, and it can be accessed by entering the following in Terminal:
...
Sequence and SNP array data generally take the form of unphased genotypes (i.e., we do not know which alleles are on the same haplotype). You will need to phase the dataset to use most rare variant analysis methods, because the calculating the estimates requires knowing which parent transmitted which alleles to the child.
...
```{r, eval=FALSE}
filepath.ped<-"/Users/lindagai 1/Documents/classes/4th year/Research/rvtrio/data/gmkf_euro_completetrios_ids_match_vcf_mend_errors_removed.phen"
ped <- read.table(filepath.ped,header=FALSE)
head(ped)
```
```{r, echo=FALSE}
filepath.ped<-"/Users/lindagai 1/Documents/classes/4th year/Research/rvtrio/data/gmkf_euro_completetrios_ids_match_vcf_mend_errors_removed.phen"
...
2. VCF file for common variants analysis

    + all PIDs in VCF are present in PED file and vice versa
    + families with large numbers of Mendelian errors are removed
    + genotype entries are in `"0/0"`, `"1/0"`, or `"1/1"` format; missing data is indicated by an `NA`
...
The `geno` object can be created using the `vcf2geno` function.
```{r, eval=FALSE}
trio.geno <- trio::vcf2geno(vcf,ped)
```
```{r,echo = FALSE}
filepath.geno<-"/Users/lindagai 1/Documents/classes/4th year/Research/rvtrio/data/8q24_trio_geno_07_31_19.RDS"
trio.geno<-readRDS(filepath.geno)
```
It is recommended to limit the common variants analysis to common variants, i.e. SNPs with MAF > 0.05:

```{r}
trio.geno <- trio::removeSNPs(trio.geno, maf = 0.05)
```

The geno matrix itself is a matrix, which has:
...
```{r, eval=FALSE}
filepath.trio.geno<-"/users/lgai/8q24_project/data/processed_data/trio_tdt_format/8q24_trio_geno_07_019_19.RDS"
...
```{r, eval=FALSE}
#This also takes awhile, you may want to save it
gTDT.results<-trio::tdt(trio.geno, model = c("additive"))
gTDT.results
```
```{r,echo = FALSE}
filepath.gTDT.results<-"/Users/lindagai 1/Documents/classes/4th year/Research/rvtrio/data/8q24_gTDT_07_31_19.RDS"
gTDT.results<-readRDS(filepath.gTDT.results)
gTDT.results
```
...
```
```{r,echo = FALSE}
filepath.aTDT.results<-"/Users/lindagai 1/Documents/classes/4th year/Research/rvtrio/data/8q24_aTDT_07_31_19.RDS"
aTDT.results<-readRDS(filepath.aTDT.results)
aTDT.results
```
```{r}
...
```{r, eval=FALSE}
filepath.common.vcf<-"/users/lgai/8q24_project/data/processed_data/vcfs/8q24.common.var.07_30_19.vcf"

## Return CHROM, POS, ID and REF from 'fixed' field
# do not read in other fields
svp <- ScanVcfParam(fixed="NA", info="NA", geno="NA")
vcf.svp <- readVcf(filepath.common.vcf, "hg19", svp)
...
```
```{r,echo = FALSE}
filepath.snp.pos.df<-"/Users/lindagai 1/Documents/classes/4th year/Research/rvtrio/data/8q24_snp.pos.df_07_31_19.RDS"
snp.pos.df<-readRDS(filepath.snp.pos.df)
head(snp.pos.df)
head(gTDT.df)
```
...
```{r, eval=FALSE}
filepath.aTDT.df<-"/where/you/want/aTDT/results/to/go"
saveRDS(aTDT.df, filepath.aTDT.results)

filepath.gTDT.df<-"/where/you/want/gTDT/results/to/go"
saveRDS(gTDT.df, filepath.gTDT.results)
```
...
Note that phasing takes a long time, so it is often preferable to do filtering steps prior to phasing (e.g., subsetting the VCF to a candidate gene). However, phasing uses the nearby variants to make inferences for missing observations, so there may be some benefits to phasing a whole gene at a time, as opposed to a collection of SNPs that are not necessarily close to one another.
